  ...
  <div>
    ...
    <el-row class="table-mains">
      ...
      <el-col class="tm-top" style="justify-content: flex-start;" :offset="1" :xl="17" :lg="14">
        <div>
          已买产品：
          <el-select v-model="isCValue" @change="handleChange(4)" multiple collapse-tags placeholder="请选择已买产品">
            <el-option
              v-for="item in isCList"
              :key="item.id"
              :disabled="item.noChose"
              :label="item.name"
              :value="item.id">
            </el-option>
          </el-select>
        ...
        <div style="margin-left:20px;">
          未买产品：
          <el-select v-model="noCValue" @change="handleChange(5)" multiple collapse-tags placeholder="请选择未买产品">
            <el-option
              v-for="item in noCList"
              :key="item.id"
              :disabled="item.isChose"
              :label="item.name"
              :value="item.id">
            </el-option>
          </el-select>
        </div>
        <!-- <el-radio-group v-model="combinationValue" @change="handleChange(1)">
          <el-radio-button v-for="(l,l_idx) in combinationList" :label="l.id" :key="l_idx">{{l.name}}</el-radio-button>
        </el-radio-group> -->
      </el-col>
      ...
      <el-col class="tm-mains" :span="24">
        <el-table
          ...
          >
            <div slot-scope="scope">
              <el-tooltip class="item" effect="dark" :content="scope.row.buyInfo || ''" placement="top">
                <span class="hiddenFonts">{{scope.row.buyInfo || '无'}}</span>
              </el-tooltip>
            </div>
          </el-table-column>
          <el-table-column
          ...
          >
            <div slot-scope="scope">
              <el-tooltip class="item" effect="dark" :content="scope.row.noBuyInfo || ''" placement="top">
                <span class="hiddenFonts">{{scope.row.noBuyInfo || '无'}}</span>
              </el-tooltip>
            </div>
          </el-table-column>
...
<script>
  import { operateScreen , areaList , combinationClue , userLevelNum , userOrder , userCycle } from '@/api/operate'
    ...
    methods: {
      ...
      getCList(){
        ...
        operateScreen(params).then(res=>{
          if(res){
            this.isCList = res.data.product_type
            this.noCList = res.data.product_type
          }
        })
      },
      // 等级切换
      ...
      handleChange(type){
        switch(type){
          ...
          break;
          case 4:
            this.noCList.forEach(v=>{
              v.isChose = false
              this.isCValue.forEach(i=>{
                if(v.id == i){
                  v.isChose = true
                }
              })
          ...
          break;
          case 5:
            this.isCList.forEach(v=>{
              v.noChose = false
              this.noCValue.forEach(i=>{
                if(v.id == i){
                  v.noChose = true
                }
              })
      ...
      tableData(type){
        ...
        let params = {
          ...
          cityName: areaList[1] || '',
          areaName: areaList[2] || '',
          com_type: this.combinationValue,
        ...
        combinationClue(params).then(res=>{
          if(type){
